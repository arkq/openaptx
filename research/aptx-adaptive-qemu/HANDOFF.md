...
> **再次更正（§21.18 / §21.19）**：本节把 `08064f` 当作 BT11 的链路，**该身份已无法复现**。
> 用受控电源循环重新锚定后，BT11 的 LAP 是 **`6a8fcc`**（普通 AD 与 Lossless 两次都一样），
> 而 `08064f` 在两种模式下**一次都没出现**。本节"BT11 链路在标准 BR/EDR 上可见"的观测，
> 以及由此推出的"耳机不要求高通控制器"，都建立在那个无法复现的锚点上，**请以 §21.18/§21.19 为准**。
...
> **更正（§21.18 / §21.19）**：本节有三处需要修正。
> ①零结果是在 `08064f`/`b7163b` 上测的，而 BT11 的 AD piconet 实为 `6a8fcc`
> —— 结论"稳态不可见"仍然成立，但当时看的是错误的 LAP；
> ②"整条 piconet 都在专有 PHY、连 ACL 信令都看不到"**过强**：(重)连接建立阶段
> 在标准 BR/EDR 上清晰可见（259 次 / 31 s）；
> ③本节把 `6a8fcc` 列为"survey 相位假象的例子"——它不是假象，它就是 BT11 的链路。
> 另外"Lossless 走标准 EDR"这一半在 2026-09-12 下午用同样方法**无法复现**（§21.19）。
...
### 21.20 用户确认 dongle 必在 Lossless ⇒ 结论改写（2026-09-12 傍晚）

用户明确：**BT11 必然在跑 Lossless**。因此 §21.19 的两种解释取 **(a)**：
旧锚点 `08064f` 是误判，`6a8fcc` 才是 BT11，**Lossless 与 AD 一样，在稳态离开标准 BR/EDR**。

**补做的活性对照（304 s 稳态长抓包，Lossless + 音乐在播）**：

| 指标 | 数值 |
|---|---|
| 抓到总包数 / piconet 数 | **535 包 / 32 个 piconet**（仪器确实在工作） |
| 最活跃的几个 | `17f122` 78、`ee02f8` 78、`c1b781` 65、`6f18bb` 57 |
| `6a8fcc`（BT11） | **0** |
| `08064f`（旧锚点） | **0** |
| `b7163b`（耳机） | **0** |

累计"音频在播但 BT11 链路不可见"的时长：**Lossless 551 s**（118+129+304）、
**AD 488 s**（118+180+190），全部经门禁与宿主侧 `node.state=running` 确认。

**结论改写（这是本项目最重要的一次结论反转）**：

1. **三个可听源（手机 AD、BT11 AD、BT11 Lossless）在稳态下都不在标准 BR/EDR 上**；
   而唯一"标准 EDR 可见且正在播放"的源（本机 AX210 的 AD）**静音**。
   ⇒ "耳机需要专有高速链路（QHS 一类）才能解码"成为**唯一有观测支持的读法**。
2. 旧推理链中"BT11 的 Lossless 走标准 EDR 且能出声 ⇒ 耳机不要求高通控制器、
   只要求它认识的码流形态"这一环，**失去唯一支撑观测**。§21.13 的"QHS 不是门槛"
   与 §21.15 的"Lossless 走标准 EDR"据此作废（两节均已加更正标记）。
3. 与之独立的一条仍然成立且互相印证：§21.16 把 R2.2/Snapdragon Sound 形态
   （`0xaf`、ptype 5、760 B 帧、chan `0xa0`）**连续**发上过标准 EDR 空口，
   耳机依旧静音 ⇒ **"码流形态对了就能出声"在这台控制器上不成立**。
4. **对项目目标的影响（诚实结论）**：原目标"在本机 Intel 控制器上实现普通 aptX Adaptive"
   卡在**链路层**而非码流层。要达成需要：能驱动该专有高速链路的控制器（高通 FastConnect
   一类，或带自家 offload 的高通 SoC 方案），或找到耳机在标准 EDR 下接受该码流的条件
   （目前没有任何观测支持后者存在）。
5. 遗留的**唯一**未测分支：模块能否在 ABR=0 下产出**版本字节 `0xad`（R3）**的连续流
   （`APTX_OTA_VERSION` 可强制该字节）。即使能出流，按第 1、3 条它仍会因链路层而静音，
   因此优先级低于"换控制器"。

**方法论沉淀（新增两条）**：
1. **负结果必须带活性对照**：本次 304 s 的"0 次"之所以可信，是因为同一段抓包里
...
### 21.24 P1 实测：把"专有 PHY"假设与测量伪影分开（2026-09-12 中午）

**背景**：§21.23(十) 把"能出声的源在标准 BR/EDR 上不可见"降级为低–中置信度，理由是
两条平凡解释：`ubertooth-rx` 不应用 AFH 信道图；以及同行评审研究显示标准 A2DP 链路
被动抓包只恢复到源端 HCI 的约 1/200。本轮（P1）用**近距离 + 同仪器阳性对照**把这两条
逐条测掉。

**实验设计（关键是"同一台仪器、同一副耳机、同一时段"）**

- 仪器：Ubertooth One，**survey 模式**（`ubertooth-rx -z -t N`）。survey 会扫过全部
  79 个信道，**因此 AFH 信道图缺失对它不适用**——这一点是 §21.23(十) 第 1 条被错误
  推广的地方：AFH 顾虑只适用于 follow/camp 模式。
- 手机**贴在** Ubertooth 天线上播放 aptX Adaptive 48 kHz（用户确认耳机里能听到声音）。
- **阳性对照**：把同一副耳机接到本机 AX210，用 **aptX HD** 推流（标准 EDR、可出声），
  同一仪器、同一房间、同一 sink。
- 补充格：本机推 **aptX Adaptive**（标准 EDR、静音）与本机**空闲**两种状态。

**结果（全部为 2026-09-12 12:12–12:28 实测）**

| 状态 | 时长 | 手机 `40fddd` | 本机 `c50142` | 耳机 `b7163b` |
|---|---|---|---|---|
| 手机单独（本机蓝牙关） | 109 s | **0** | — | 0 |
| 手机单独（本机空闲）C1 | 99 s | **0** | 2 | 0 |
| 手机单独（本机空闲）C2 | 99 s | **0** | 2 | 0 |
| 本机 aptX HD 推流 D0 | 109 s | 0 | **160** | 0 |
| 本机 aptX HD 推流 D1 | 99 s | 0 | **93** | 0 |
| 本机 aptX HD 推流 A | 98 s | 0 | **83** | 0 |
| 本机 aptX HD 推流 B | 100 s | 0 | **68** | 0 |
| **本机 aptX Adaptive 推流（静音）** | 100 s | 0 | **52** | 0 |

- 手机**出声播放累计 307 秒，0 命中**；按阳性对照的 0.68–1.60 命中/秒推算，
  若它以同样方式发射，**预期 215–460 次**。
- 本机链路**空闲 2/100 s、推流 52–93/100 s ⇒ 可见性是媒体驱动的**，不是链路维持流量。
- 独立模式复核：`ubertooth-rx -l 40fddd -t 60`（只针对该 LAP 反推 UAP）⇒ **0 包**。
- 同一批抓包里其他 piconet 大量可见（`9ba8a3` 67、`6a8fcc` 71、`134eb9` 121 …），
  仪器活性无疑。

**结论**

1. **"手机的 aptX Adaptive 媒体不出现在标准 BR/EDR 上"——置信度回到"高"**
   （比 §21.20 当时的依据强得多：那时是 follow/camp 与锚定对照，现在是
   **survey + 同场阳性对照 + 307 秒零命中**）。
2. §21.23(十) 的两条平凡解释**都被实测排除**：AFH 不适用于 survey 模式；而
   "标准 A2DP 抓不到"被**同场阳性对照直接反驳**（同仪器、同 sink，标准 EDR 的
   HD 链路 0.68–1.60 命中/秒）。
3. 我们**自己**的 AD 链路是可见的（52/100 s）却静音 ⇒ 结论不是"离开 PHY 就能出声"，
   而是"**耳机只从它认识的那条（厂商）通路取 AD 音频**"。
4. 保留的限制：(i) Ubertooth **不能解 EDR 载荷**，所以我们能说的是"**没有检测到
   该 LAP 的接入码**"，不能说"知道它发的是什么"；(ii) 手机实际用什么调制仍未测
   （需 2.4 GHz SDR，如 ADALM-Pluto/HackRF）；(iii) 本机 AD 链路可见而静音这一格
   说明"可见性"本身不是出声的充分条件。

**方法论沉淀（五条）**

1. **说清仪器的模式边界**：`-z` survey 与 `-l/-u` follow 的失效模式不同，
   把 follow 的 AFH 缺陷套到 survey 上会误伤一个有效结论。
2. **阳性对照必须同场、同 sink、同仪器**：借鉴同行评审的悲观数字（200× 落差）时，
   要先用本地对照测出"本仪器在本场景下的实际灵敏度"，否则会把有效负结果扔掉。
3. **"空闲 vs 推流"能把"没看到"变成"没看到本该看到的东西"**：这条是本次让 0 命中
   变得有力的关键。
4. **用户的提醒必须写进协议**：多点连接（multipoint）下耳机同时只播一个源，

...
uint32_t aptXHD_pack_codeword(aptXHD_subband_encoder_100 *e) {
	int x = 1 & (
			...
			e->dither_sign);
	return
		((e->quantizer[0].unk1 & 0x1FF) << 0) |
		((e->quantizer[1].unk1 & 0x3F) << 9) |
		((e->quantizer[2].unk1 & 0x0F) << 15) |
		(((e->quantizer[3].unk1 & 0x1E) | x) << 19);
}
...
int32_t aptXHD_insert_sync(
	...
	if (x != ((1 >> *sync) & 1)) {
		...
		const size_t map[__APTXHD_SUBBAND_MAX] = { 1, 2, 0, 3 };
